...
describe("auth.forgotPassword (验证码 + 新密码 一站式重置)", () => {
    it("POST /api/v1/auth/forgot-password 含 email+code+new_password，返回 detail 文案", async () => {
        const fetchMock = mockFetchOk({ detail: "密码已重置，请使用新密码登录" });
        vi.stubGlobal("fetch", fetchMock);

        const detail = await forgotPassword("user@example.com", "123456", "NewPass123!");
    ...
    it("无 Authorization 头（公开端点，用户未登录）", async () => {
        ...
        await forgotPassword("user@example.com", "123456", "NewPass123!");
    ...
    it("后端 400（验证码错误）抛异常含错误消息", async () => {
        const fetchMock = vi.fn(async () =>
            new Response(JSON.stringify({ detail: "验证码无效或已过期" }), { status: 400 })
        );
        vi.stubGlobal("fetch", fetchMock);

        await expect(
            forgotPassword("user@example.com", "000000", "NewPass123!")
    ...
    it("后端 422（密码强度不足）抛异常含错误消息", async () => {
        ...
        await expect(
            forgotPassword("user@example.com", "123456", "short")
        ).rejects.toThrow("密码至少8位");
    });
});
