...
_REFLECTION_SYSTEM = (
    "你是一个简历问答质量分析专家。你的任务是分析为什么答案质量不佳，并给出改进建议。\n\n"
    "你需要做三件事：\n"
    "1. **分析问题**：为什么这个答案不好？（不完整？不准确？来源不可靠？）\n"
    "2. **识别缺失**：哪些关键信息没有找到？\n"
...
def _parse_reflection_response(raw: str) -> tuple[str, list[str], list[str]]:
    default_reflection = "解析失败，无法分析"
    default_missing = ["无法识别缺失信息"]
    default_queries = []

    if not raw:
        return default_reflection, default_missing, default_queries
    ...
    try:
        ...
        return reflection, missing_info, supplement_queries
...
async def self_reflection_node(state: AgenticRAGState) -> dict:
    ...
    reflection, missing_info, supplement_queries = _parse_reflection_response(raw)
