...
def test_h5_mcp_graph_should_route_to_self_reflection():
    ...
    assert has_reflection_in_std, (
        "标准 Graph 应包含 SELF_REFLECTION 路由（前置条件）"
    )
    assert has_reflection_in_mcp, (
        "MCP Graph 应包含 SELF_REFLECTION 路由，"
        "但当前直接跳回 MCP_SEARCH_NODE 跳过了反思步骤"
    )
...
def test_h5_mcp_route_retry_should_not_skip_reflection():
    ...
    if retry_pattern:
        ...
        assert "SELF_REFLECTION" in target, (
            f"重试时路由目标应为 SELF_REFLECTION_NODE，"
            f"当前为 {target}"
        )