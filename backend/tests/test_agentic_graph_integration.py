...
class TestIntegrationSearchPath:
    """集成测试：search 路径，真实节点逻辑 + mock 底层 LLM client + mock 检索。"""
...
class TestIntegrationDirectAnswer:
    """集成测试：direct_answer 路径只 mock LLM client for rewrite。"""
    ...
    async def test_direct_answer_with_real_rewrite(self):
        ...
        with patch(
            ...
            result = await graph.ainvoke(
                ...
                config={"configurable": {"thread_id": "integration-direct"}},
            )
...
class TestIntegrationReflexion:
    """集成测试：Reflexion 循环，真实 evaluate 评分解析 + 真实 self_reflection。"""
    ...
    async def test_reflexion_with_real_evaluate_parsing(self):
        ...
        with patch(
            ...
            result = await graph.ainvoke(
                ...
                config={"configurable": {"thread_id": "integration-reflexion"}},
            )
