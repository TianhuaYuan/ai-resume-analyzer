...
class TestParseReflectionResponse:
    ...
    def test_valid_json(self):
        ...
        reflection, _, _, missing, queries, _ = _parse_reflection_response(raw)
    ...
    def test_empty_string(self):
        reflection, _, _, missing, queries, _ = _parse_reflection_response("")
    ...
    def test_malformed_json(self):
        """损坏的 JSON 应降级。"""
        reflection, _, _, missing, queries, _ = _parse_reflection_response("{broken json")
        assert reflection == "解析失败，无法分析"
    ...
    def test_limits_supplement_queries(self):
        ...
        _, _, _, _, result_queries, _ = _parse_reflection_response(raw)
        assert len(result_queries) == _MAX_SUPPLEMENT_QUERIES

    def test_float_values(self):
        """浮点数应正确处理。"""
        raw = '{"reflection": 123, "missing_info": [], "supplement_queries": []}'
        reflection, _, _, _, _, _ = _parse_reflection_response(raw)
